...
## 7. TDD Workflow (per NFR1)

*   **Backend:** For every new feature (e.g., a data validation rule), a corresponding test case must be written in `pytest` *first*. The test will fail initially, and then the feature code will be written to make it pass.
*   **Frontend:** For every new component or piece of logic (e.g., form validation), a test must be written in `vitest` or `jest` *first*.
